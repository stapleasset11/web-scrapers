...
class IndeedJobSpider(scrapy.Spider):
    ...
    def parse_search_results(self, response):
        ...
        ray_id = response.headers.get('rayid', None)
        ...
        if script_tag is not None:
            ...
            for index, job in enumerate(jobs_list):
                if job.get('jobkey') is not None:
                    ...
                    headers = {
                        ...
                        'rayid': ray_id.decode('utf-8') if ray_id else None,  # Include Ray ID if present
                    }
                    yield scrapy.Request(url=job_url, callback=self.parse_job, headers=headers, meta={
                        'keyword': keyword,
                        'location': location,
            ...
            if offset == 0:
                ...
                for new_offset in range(10, num_results + 10, 10):
                    ...
                    yield scrapy.Request(url=url, callback=self.parse_search_results, meta={'keyword': keyword, 'location': location, 'offset': new_offset})
    ...
    def parse_job(self, response):
        ...
        if script_tag is not None:
            json_blob = json.loads(script_tag[0])
            job = json.loads(script_tag[0])["jobInfoWrapperModel"]["jobInfoModel"]['jobInfoHeaderModel']
            sanitizedJobDescription = json.loads(script_tag[0])["jobInfoWrapperModel"]["jobInfoModel"]['sanitizedJobDescription']
            yield {
                'keyword': keyword,
                'location': location,
                'page': page,
